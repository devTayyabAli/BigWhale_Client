...
import { getVerifyEmailOTP } from "src/store/apps/auth/getAccountVerifyEmailOTPSlice";
import { verifyEmailOTP } from "src/store/apps/auth/verifyAccountEmailOTPSlice";
...
const VerifyAccount = () => {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const response = await dispatch(getVerifyEmailOTP({ email: userOTPEmail }));
  ...
  const onSubmit = async (values) => {
    ...
    try {
      const response = await dispatch(
        verifyEmailOTP({ email: userOTPEmail, otp: accountVerificationCode })
      );
      if (response?.meta?.requestStatus === "fulfilled") {
